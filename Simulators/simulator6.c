// commad to compile: gcc -o simulator6 simulator6.c coursework.c linkedlist.c -lpthread 
// command to run: ./simulator6 <Simulators>     #i-e  [./simulator6 4]->for 4 simulators
    ...
    if(count==1){
        ...
        while(pCurrent != NULL) {
            ...
            if((pProcess1->iTrack - *cur)<prevdeff && (pProcess1->iTrack - *cur)>0){
                printf("LOOK-SCAN: [PID = %d, Track = %d, Direction = %d]\n", pProcess1->iPID, pProcess1->iTrack, *direction);
